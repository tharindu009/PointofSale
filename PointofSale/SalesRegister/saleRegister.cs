...
{
    public partial class saleRegister : KryptonForm
    {
        /// <summary>
        ...
        public saleRegister(string JobNo)
        {
            InitializeComponent();
            lbluser.Text = UserInfo.UserName;
            //  this.tabPageSR_Payment.Parent = null; //Hide payment tab
        ...
        private void SalesRegister_Load(object sender, EventArgs e)
        {
            ...
            if (CurrentJobNo != "0")
            {
                ...
                try
                {
                    string dataDS = "SELECT JobNo,EngineOil,GearOil,OilFilter,AirFilter,FuelFilter,ATFFilter,CabinFilter,VehicleNo,DiffOil,VehicleNo FROM job_card where JobNo = '" + CurrentJobNo + "'";
                    ...
                    if (rows > 0)
                    {
                        ...
                        BindItemJobCard(EngineOil);
                        if (GearOil != "None" || GearOil != "")
                        {
                            BindItemJobCard(GearOil);
                        }
                        if (OilFilter != "None")
                        {
                            BindItemJobCard(OilFilter);
                        }
                        if (AirFilter != "None")
                        {
                            BindItemJobCard(AirFilter);
                        }
                        if (FuelFilter != "None")
                        {
                            BindItemJobCard(FuelFilter);
                        }
                        if (CabinFilter != "None")
                        {
                            BindItemJobCard(CabinFilter);
                        }
                        if (DiffOil != "None")
                        {
                            BindItemJobCard(DiffOil);
                        }
        ...
        private void tabPageSR_Payment_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                ...
                else
                {
                    try
                    {
                        ...
                        SalesRegister.POSPrintRpt go = new SalesRegister.POSPrintRpt(txtInvoice.Text);
                        go.ShowDialog();
